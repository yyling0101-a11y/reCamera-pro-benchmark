...
def main():
    parser = argparse.ArgumentParser(description="YOLO11 Seg RKNN inference")
    ...
    print(f"\n{'='*60}")
    print(f"YOLO11 Seg RKNN Inference")
    print(f"{'='*60}")
    ...
    print(f"\nPost-processing...")
    detections = postprocess_seg(last_outputs, ratio, (dw, dh),
    ...
    for i, d in enumerate(detections[:10]):
        ...
        ma = int((d['mask'] > 0.5).sum()) if d.get('mask') is not None else 0
        print(f"  [{i}] {cls_name}: {d['conf']:.3f} [{b[0]:.0f},{b[1]:.0f},{b[2]:.0f},{b[3]:.0f}] mask={ma}px")
