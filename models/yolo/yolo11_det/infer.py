...
Usage:
    python infer_yolov8_zoo.py \
        --model yolov8n_zoo_W8A8.rknn \
...
def main():
    parser = argparse.ArgumentParser(description="YOLO11 Det RKNN inference")
    ...
    print(f"\n{'='*60}")
    print(f"YOLO11 Det RKNN Inference")
    print(f"{'='*60}")
    ...
    print(f"\nPost-processing...")
    detections = postprocess_det(last_outputs, ratio, (dw, dh),
    ...
    for i, d in enumerate(detections[:10]):
        ...
        print(f"  [{i}] {cls_name}: {d['conf']:.3f} [{b[0]:.0f},{b[1]:.0f},{b[2]:.0f},{b[3]:.0f}]")
    img_draw = draw_det(orig_img.copy(), detections)
