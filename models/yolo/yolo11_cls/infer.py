...
def main():
    parser = argparse.ArgumentParser(description="YOLO11 Classification RKNN inference")
    ...
    print(f"\n{'='*60}")
    print(f"YOLO11 Classification RKNN Inference")
    print(f"{'='*60}")
    ...
    print(f"\nPost-processing...")
    dets = postprocess_cls(last_outputs, ratio, (dw, dh), imgsz=args.imgsz)
    print(f"Top-5:")
    for i, d in enumerate(dets):
        print(f"  [{i}] class_{d['class']}: {d['conf']:.4f}")
    img_draw = draw_cls(orig_img.copy(), dets)
